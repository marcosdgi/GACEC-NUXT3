...
const {data: actividadesEconomicas, execute} = useAsyncData(async()=>{
  return await $api.actividadEconomica.obtenerActividadesEconomicas()
})
...
.grid.items-start.w-full.bg-white.p-4.rounded-md(class="h-[600px]")
    ...
    hr.mt-2
    .grid.items-center.p-2(v-if='actividadesEconomicas?.length')
        ClasificacionesActividadesEconomicasPrincipalesActividadEconomicaPrincipalTable(:actividades-principales="actividadesEconomicas")
        ClasificacionesActividadesEconomicasPrincipalesActividadEconomicaPrincipalDrawer(
