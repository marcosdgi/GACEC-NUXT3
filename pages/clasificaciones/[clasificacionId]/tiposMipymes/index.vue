...
const {data: tiposMipymes, execute} = useAsyncData(async()=>{
  return await $api.tipoMipyme.obtenerTiposMipymes()
})
...
.grid.items-start.w-full.bg-white.p-4.rounded-md(class="h-[600px]")
    .flex.flex-row.items-center.gap-3.h-10
            ...
            p.text-3xl.font-semibold.text-green-800 Tipos de Mypimes
            ElTooltip(content="Agregar nuevo tipo de mipyme" placement="top-start")
                ElButton(type="primary" circle @click="handleDrawerOpen" size="large").ml-2.shadow-xl
                    Icon(name="tabler:plus" size="35")
    hr.mt-2
    .grid.items-center.p-2(v-if="tiposMipymes?.length")
        ClasificacionesTiposMipymesTipoMipymeTable(:tipos-mipymes="tiposMipymes")
        ClasificacionesTiposMipymesTipoMipymeDrawer(
