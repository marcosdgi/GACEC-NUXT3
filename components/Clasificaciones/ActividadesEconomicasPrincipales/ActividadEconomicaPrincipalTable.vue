...
interface Props{
  actividadesPrincipales: Array<IActividadEconomicaResponse>
}
const props = defineProps<Props>()
const actividadesPrincipalesRef = ref<Array<IActividadEconomicaResponse>>(props.actividadesPrincipales)
let columns = [
  {key:"id", dataKey:"id", title: "No.", width:80},
  {key:"Codigo", dataKey:"codigo", title: "Código", width:100},
  {key:"Nombre", dataKey:"nombre", title: "Nombre", width:300},
  {key:"Descripcion", dataKey:"descripcion", title: "Descripción", width:300},
]
watch(()=>props.actividadesPrincipales, (newValue)=>{
  actividadesPrincipalesRef.value = newValue
},{deep:true})
...
const cellProps = ({ column, rowData, rowIndex }: any) => {
  ...
  return {
    ...
    //   Cambiar por el id de la clasificacion durante el acceso a los datos
      router.push(`/clasificaciones/${clasificacionId}/actividadesEconomicasPrincipales/actividadEconomicaPrincipal/${rowData.id}`)
    }
  }
}
...
<template>
  <div v-if="actividadesPrincipalesRef.length" style="height: 400px" class="border-2 border-green-600 rounded-md p-1">
    <el-auto-resizer>
      <template #default="{ height, width }">
        <el-table-v2
          ...
          :class="kls"
          :data="actividadesPrincipalesRef"
          :width="width"
          :height="height"
        />
      </template>
