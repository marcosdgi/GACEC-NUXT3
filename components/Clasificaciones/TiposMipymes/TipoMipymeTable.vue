...
watch(()=>props.tiposMipymes, (newValue)=>{
  tiposMipymesRef.value = newValue
},{deep:true})
...
const cellProps = ({ column, rowData, rowIndex }: any) => {
  ...
  return {
    ...
    //   Cambiar por el id de la clasificacion durante el acceso a los datos
      router.push(`/clasificaciones/${clasificacionId}/tiposMipymes/tipoMipyme/${rowData.id}`)
    }
  }
}
...
<template>
  <div v-if="tiposMipymesRef" style="height: 400px" class="border-2 border-green-600 rounded-md p-1">
    <el-auto-resizer>
  ...
  <el-empty v-else description="No hay tipos de mipymes registrados en el sistema"/>
</template>
<style>
