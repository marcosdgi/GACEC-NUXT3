...
const sectoresEconomicosRef = ref<Array<ISectorEconomicoResponse>>(props.sectoresEconomicos)
let columns = [
...
watch(()=>props.sectoresEconomicos, (newValue)=>{
  sectoresEconomicosRef.value = newValue
},{deep:true})
...
const cellProps = ({ column, rowData, rowIndex }: any) => {
  ...
  return {
    ...
    //   Cambiar por el id de la clasificacion durante el acceso a los datos
      router.push(`/clasificaciones/${clasificacionId}/sectoresEconomicos/sectorEconomico/${rowData.id}`)
    }
  }
}
...
<template>
  <div v-if="sectoresEconomicos.length" style="height: 400px" class="border-2 border-green-600 rounded-md p-1">
    <el-auto-resizer>
  ...
  <el-empty v-else description="No hay sectores económicos registrados en el sistema" />
</template>
<style>
