...
const denominacionesRef = ref<Array<IDenominacionResponse>>(props.denominaciones)
let columns = [
...
watch(()=>props.denominaciones, (newValue)=>{
  denominacionesRef.value = newValue
},{deep:true})
const cellProps = ({ column, rowData, rowIndex }: any) => {
  ...
  return {
    ...
    //   Cambiar por el id de la clasificacion durante el acceso a los datos
      router.push(`/clasificaciones/${Number(clasificacionId)}/denominaciones/denominacion/${rowData.id}`)
    }
  }
}
...
<template>
  <div v-if="denominaciones.length" style="height: 400px" class="border-2 border-green-600 rounded-md p-1">
    <el-auto-resizer>
  ...
  <el-empty v-else description="No hay denominaciones registradas en el sistema"/>
</template>
<style>
