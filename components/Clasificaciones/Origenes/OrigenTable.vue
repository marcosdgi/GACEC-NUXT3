...
const origenesRef = ref<Array<IOrigenResponse>>(props.origenes)
let columns = [
...
watch(()=>props.origenes, (newValue)=>{
  origenesRef.value = newValue
},{deep:true})
...
const cellProps = ({ column, rowData, rowIndex }: any) => {
  ...
  return {
    ...
    //   Cambiar por el id de la clasificacion durante el acceso a los datos
      router.push(`/clasificaciones/${clasificacionId}/origenes/origen/${rowData.id}`)
    }
  }
}
...
<template>
  <div v-if="origenesRef.length" style="height: 400px" class="border-2 border-green-600 rounded-md p-1">
    <el-auto-resizer>
  ...
  <el-empty v-else description="No hay orígenes registrados en el sistema"/>
</template>
<style>
